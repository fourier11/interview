...
/**
 * 给定一个类，内含同步方法A和同步方法B。在同一个程序实例中，有两个线程，能否同时执行A，两者能否同时执行A和B
 * 视情况而定，如果两个线程拥有该对象的同一个实例，那么答案是否定的，它们不能同时执行方法。如果是不同实例，则可以。
 * 第二个问题，不管thread1和thread2是否拥有该对象的统一实例，这一点都是成立。
 */
